...
def run_mongo_command():
    ...
    result_dict["minball2018"] = list()
    result_dict["minball2021"] = list()

    agg_result_for_2018 = coll.aggregate(
        [
            {"$match": {"$or": [{"mathteststatus": "Зараховано", "year": 2018}]}},
            {"$group": {"_id": "$regname", "minball": {"$min": "$mathball100"}}}
        ])

    agg_result_for_2021 = coll.aggregate(
        [
            {"$match": {"$or": [{"mathteststatus": "Зараховано", "year": 2021}]}},
            {"$group": {"_id": "$regname", "minball": {"$min": "$mathball100"}}}
        ])

    for line in agg_result_for_2018:
        result_dict["regname"].append(line["_id"])
        result_dict["minball2018"].append(line["minball"])

    for line in agg_result_for_2021:
        result_dict["minball2021"].append(line["minball"])
    ...
    for name in filename_list:
        ...
        if file_counter < logged_numbers[0]:
            ...
            try:
                ...
                for chunk in pd.read_csv(f"../zno/{name}", sep=';' , encoding='utf-8' , on_bad_lines='skip', chunksize=chunk_size,
                                         low_memory=False):
                    chunk_counter += 1
